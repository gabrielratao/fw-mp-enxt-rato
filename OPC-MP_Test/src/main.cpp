...
{
  SD.begin(chipSelect);
  myFile = SD.open("mpSensor.txt", FILE_WRITE);
  //myFile.print("testeVoid");
  ...
  for (i=60; i<72; i+=4)
  {
    ...
    myFile.println(*pFloat, 3);
    port.print(*pFloat, 3); //print to 3dp
    myFile.close();
    Serial.print("a principio gravou no sd");
  }
...
void setup()
{
  ...
  //myFile = SD.open("mp.txt", FILE_WRITE);
